...
flowchart TD
    ...
    EVENT["CI event (auto-detected)\nGitLab CI_* env vars / GitHub event JSON\nor --src-repo/--base/--head"]
...
> explicit `--from-event` > explicit `--src-repo/--base/--head` flags > CI auto-detect (`$GITLAB_CI` → GitLab `CI_*` env vars; `$GITHUB_EVENT_PATH` → GitHub event JSON)

This is what makes CI "just work": in CI, repo / base / head / PR are derived automatically with no flags, and the platform is auto-detected (`diff_from_ci`) — GitLab exposes `CI_*` environment variables (signalled by `$GITLAB_CI`), while GitHub writes an event JSON at `$GITHUB_EVENT_PATH`. Locally, `_build_diff` decides between a local checkout (`diff_local`, when `src_repo` is a path containing `.git`) and a GitHub fetch (`diff_github`).

A manual run that passes `--src-repo` and `--head` but omits `--base` falls back to the stored cursor: `_base_or_cursor` resolves the last `head_sha` docsync processed for that repo (matched by `_repo_key` normalization, so a bare name still matches an `owner/name` cursor) and uses it as the "since last sync" base — raising a `BadParameter` when no cursor exists yet. The `map` command shares this fallback.

The repo topology shapes what reaches impact mapping. `cfg.resolve_repo_mode` resolves an effective `mono` / `single` / `poly` mode (explicit `config.repo_mode`, else auto-detected from whether the diff's repo *is* the docs repo and how many source repos the manifest spans). In a **mono** repo — docs and code sharing one checkout — the resolved diff also carries the docs subtree, so `run` filters it through `impact.filter_docs_paths`, dropping files under `docs_root` so a merged doc change can't map onto itself and drive further edits.
...
A `thoroughness` dial (`light | medium | high`, from `config.thoroughness` / `config.thoroughness_by_kind` or the `--thoroughness` flag) tunes how much gets written: in bootstrap it sizes the plan (a small essential spine vs. a page per subsystem) and each page's depth; in the update flow it sets edit depth and the diff-size token budget.
...
An empty plan is never a valid outcome: because `DocPlan.pages` defaults to `[]`, a model that under-fills the schema would otherwise "succeed" into a silent 0-planned run. `plan_docs` instead raises a `RuntimeError` naming the judge model and pointing at the config / `DOCSYNC_LLM_DEBUG` knobs an operator behind a gateway needs to diagnose it.
...
### 6 · Self-critique (update gate, on by default)

Self-critique runs by default (disable with `--no-self-critique`, or via `config.self_critique`): each proposed `PageEdit` passes through an adversarial second opinion before validation. `critique.critique_page_edit` runs a cheap Haiku call that judges every edit op against the actual diff and asks one question only: *does this edit faithfully reflect THIS diff, and touch only what the diff changed?*
...
- **Update flow:** `validate_page` checks each surviving edit. Validation runs through a docs-format **adapter** (`validate.get_adapter`) that knows how to parse the target format and resolve nav routes. The active adapter is selected by `config.adapter` — `mintlify` (`.mdx` + MDX components, `docs.json` nav) or `markdown` (plain `.md`, YAML frontmatter, no nav manifest) — and every stage resolves it through the `adapters` registry's `make_adapter`.
...
An optional broken-link soft gate runs when `--check-links` is set — it runs the active adapter's link checker, which is a no-op for adapters without one (e.g. plain markdown).
...
<Tabs>
<Tab title="--open-pr">
`pr.open_pr` branches, commits, pushes, and opens a docs **PR (GitHub, via `gh`)** or **MR (GitLab, via `glab`)** — with a generated title (`docs: sync with <repo> #<pr>`), the markdown report as the body, and `reviewers` / `pr_labels` from config. The host is chosen by `config.forge` (`auto | github | gitlab`); `auto` detects it from the docs repo's `origin` remote (URLs containing `gitlab` → GitLab, else GitHub). On a re-run, an open PR/MR on the same branch is updated in place rather than duplicated.
</Tab>
...
The `--backend` flag (or the `backend` config field) chooses where the client comes from: `api` (an `ANTHROPIC_API_KEY` client), `claude-code` (reusing local Claude Code CLI auth, no API key), or `cursor` (the Cursor CLI `cursor-agent`, reusing your Cursor subscription). Both response shapes — the SDK's `usage` object and the `claude-code` CLI envelope's `usage` dict — are handled by the same metering code; the `cursor` backend reports no usage, so its runs show no cost estimate.
...
`generate_page_edit` proposes surgical `PageEdit`s; an adversarial Haiku pass (on by default; `--no-self-critique` disables) drops unfaithful ops.
</Step>
<Step title="Validate">
