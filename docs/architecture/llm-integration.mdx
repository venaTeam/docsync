...
A backend selector — `get_client` — chooses between them, returning the production `anthropic.Anthropic()` client for real runs and the dev-only `ClaudeCodeClient` or `CursorClient` for local dogfooding. Whichever it returns, the calling stage code is identical, because all three honor the `messages.parse(...)` / `parsed_output` contract above.
