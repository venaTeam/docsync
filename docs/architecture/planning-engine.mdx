...
<Steps>
  ...
  <Step title="One structured planner call">
    Inside `cost.stage("plan")`, the client is asked to `parse` the response into a `DocPlan` (`output_format=DocPlan`, capped at `_PLAN_MAX_TOKENS` = 6,000). The model is forced into the schema, so no brittle text parsing is needed.
  </Step>
