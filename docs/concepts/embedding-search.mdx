...
Indexing, caching, and querying never import `torch` or `sentence-transformers` directly — they only call whatever `Encoder` they're handed. That decoupling has three payoffs:
...
- **Air-gapped loading** — `default_encoder()` runs `model_name` through `resolve_model_source()` first, so a copy of the default model vendored into the package loads locally with no HuggingFace call.
...
<Note>
When the wheel was built with the model **vendored** into the package (`docsync/_models/<name>`, populated by `scripts/vendor_model.py`), `resolve_model_source` maps the default model id to that local copy — an air-gapped install loads the recall-net offline with zero config and no HuggingFace call. An explicit local path or non-default id passes through unchanged; with nothing vendored the id resolves online as usual. Cache keys stay keyed on the logical model id, so on-disk indexes are unaffected.
</Note>
...
The cache-check/build/save logic lives in a shared `_cached_or_build` helper used by both the doc and code index loaders. A cached index is reused **only** if both its content hash and model name still match. Any drift in the docs (or a model swap) invalidates the cache and triggers a fresh embedding pass. Passing `cache_dir=None` disables caching entirely and always rebuilds. `DocIndex.load` is defensive: a missing file or corrupt JSON/`.npy` returns `None`, and a code index saved to the same dir is rejected too (no `page_paths` key), both falling through to a rebuild rather than an error.
