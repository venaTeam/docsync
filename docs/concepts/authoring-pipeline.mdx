...
The contract for an edit is deliberately small. The model never returns a rewritten file; it returns a `PageEdit`, which wraps a list of find/replace `EditOp`s. `generate_page_edit` is the only place in this module that talks to the Anthropic SDK for edit generation, and it is injectable with a fake `client` for tests (defaulting to `anthropic.Anthropic()`). It also accepts an optional `retry_hint`, which `build_edit_prompt` appends to the user message — the pipeline uses this to regenerate an op after a non-unique `find` was rejected.
...
<Warning>
Because matching is exact and unique, an op whose `find` isn't a verbatim, one-of-a-kind substring of the *current* working text fails hard. `EditApplicationError` carries an `ambiguous` flag that distinguishes the two failure modes: a `find` that matched **zero** times (`ambiguous=False` — not found, unfixable by re-prompting) versus one that matched **more than once** (`ambiguous=True` — non-unique, fixable by regenerating the op with more surrounding context). In the pipeline neither is fatal to the run: a not-found failure drops the page with an `"edit not applicable (dropped)"` note, while a non-unique failure earns one bounded retry — the editor is re-asked for the same edit with the `NON_UNIQUE_RETRY_HINT` appended, requesting a longer, anchored `find`. If the retry still can't apply, the page drops too.
</Warning>
...
<Steps>
  ...
  <Step title="Apply">
    `apply_edits` performs the strict str-replace. A non-unique (`ambiguous`) `EditApplicationError` triggers a single bounded retry that re-asks `generate_page_edit` with the `NON_UNIQUE_RETRY_HINT` for a longer, unique `find`; a not-found failure (or a still-failing retry) drops the page with an explanatory note.
  </Step>
