...
<Steps>
  <Step title="Doc adapter: docs_root & adapter">
    ...
    The `adapter` key selects the doc-framework adapter that owns the pages (see `docsync.adapters.ADAPTERS`): `"mintlify"` (default — `.mdx` + MDX components, `docs.json` navigation) or `"markdown"` (plain `.md` with YAML frontmatter and no navigation manifest). It drives frontmatter freeze, structural integrity, link checks, and the extension given to new pages. `docsync init --minimal --detect` auto-detects it from the docs config (`docs.json` / `docusaurus.config.js`) or the `.mdx`/`.md` tree and only writes the key when it isn't the default mintlify.
  </Step>
  ...
  <Step title="LLM backend">
    `backend` selects which LLM backend serves every stage: `"api"` (default — uses `ANTHROPIC_API_KEY`), `"claude-code"` (dev: shells to the local `claude` CLI, reusing your Claude Code auth), or `"cursor"` (dev: shells to the Cursor CLI `cursor-agent`, via `cursor-agent login` or `CURSOR_API_KEY`). The two CLI backends are for local dogfooding only; `cursor` reports no token usage, so cost metering is unavailable on its runs. The `--backend` CLI flag overrides this config value.
  </Step>
  ...
  <Step title="Self-critique">
    `self_critique` (**on by default**) runs an adversarial judge-model pass after the edit model returns its ops, dropping any op not justified by the diff — it catches non-structural hallucinations the validation gates can't (an invented flag, a described-but-absent change). It adds one cheap judge call per edited page; the CLI `--no-self-critique` flag turns it off and overrides this config value.
  </Step>
</Steps>
