...
`critique_page_edit` ties it together, mirroring the `messages.parse(...)` structured-output idiom used elsewhere in docsync:
...
- **Cost-tracked.** The call runs inside `cost.stage("critique")`, so its token spend is attributed to the critique stage.
